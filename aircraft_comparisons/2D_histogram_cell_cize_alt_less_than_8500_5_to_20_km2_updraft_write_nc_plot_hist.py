...
out_file = data_path+'netcdf_summary_files/aircraft_comparisons/ALT_UNDER_8500_Convective_cell_size_5_to_20km2_3D_WP_less_than_1eminus1_cloud_mass_less_than_1eminus6.nc'
...
out_file = data_path+'netcdf_summary_files/aircraft_comparisons/ALT_UNDER_8500_Updrafts_cell_size_5_to_20km2_3D_WP_less_than_1eminus1_cloud_mass_less_than_1eminus6.nc'
...
out_file = data_path+'netcdf_summary_files/aircraft_comparisons/ALT_UNDER_8500_TWC_cell_size_5_to_20km2_3D_WP_less_than_1eminus1_cloud_mass_less_than_1eminus6.nc'
...
#for t in np.arange(0,60.,dt_output[m]):
  ...
  for n in range(1, n_objects+1):
      #print 'checking mask and size array creation'
      #print n
      ...
      #print b
      #if a==b:
       # print 'cell size matches new values for cell size cube'
      #else:
       # print 'error, cell size not matching up' 
  del WP

  s_arrays = [cell_size_array for _ in range(49)]
  ...
  clouds_twc = cloud_mass_mc.flatten()

  print 'time_specific_size_array_size pre nan removal'
  print len(cell_mask)
  print 'time_specific_updraft_array_size pre nan removal'
  print len(ups)
  print 'time_specific_twc_array_size pre nan removal'
  print len(clouds_twc)

  cell_mask[cell_mask>20]=0
  cell_mask[cell_mask<5]=0
  #x=sizef
  #y=upf

  ups[cell_mask==0]=np.nan
  clouds_twc[cell_mask==0]=np.nan
  ...
  print len(ups)
  clouds_twc = clouds_twc[~np.isnan(clouds_twc)]
  print 'time_specific_twc_array_size'
  print len(clouds_twc)

  size_array.append(cell_mask)
  up_array.append(ups)
  twc_array.append(clouds_twc)
  
size_array_np = np.asarray(size_array)
up_array_np = np.asarray(up_array)
twc_array_np = np.asarray(twc_array)

size_array_np = size_array_np.flatten()
up_array_np = up_array_np.flatten()
twc_array_np = twc_array_np.flatten()

print 'size_array_size'
print len(size_array_np)
print 'updraft_array_size'
print len(up_array_np)
print 'twc_array_size'
print len(twc_array_np)


size_array_np =  np.concatenate(size_array_np, axis=0)
up_array_np =  np.concatenate(up_array_np, axis=0)
twc_array_np =  np.concatenate(twc_array_np, axis=0)

twc_array_np = twc_array_np*1000
...
plt.hexbin(x,y,bins='log',cmap=cmap)
plt.axis([xmin, xmax, ymin, ymax])
plt.title("updraft speed v TWC")
cb = plt.colorbar()
cb.set_label('log10(N)')
plt.ylabel('TWC (g/kg)')
plt.xlabel('Updraft speed')
plt.savefig(fig_dir+'_updraft_v_twc_in_clouds_5_to_20km2_size.png', dpi=300)
plt.show()
'''
