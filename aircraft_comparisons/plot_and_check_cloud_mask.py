...
def check_cloud_mask(data_path):
    ...
    for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
      ...
      del ice_water_mmr
      cloud_mass[cloud_mass==0]=np.nan 
      level = np.arange(10e-6,np.nanmax(cloud_mass),20)
      cmap = plt.matplotlib.colors.LinearSegmentedColormap('radar',cmaps.radar.colors,len(level))
      cmap.set_under('w')
      #cloud_mass[cloud_mass<10e-6]=0
      #lon,lat = ra.unrot_coor(pot_temperature,158.5,77.0)
      #lon = lon[x1:x2,y1:y2]
      #lat = lat[x1:x2,y1:y2]
      h = pot_temperature.coord('level_height').points[:]
      lon = pot_temperature.coord('grid_longitude').points[:]
      

      height,lons = np.meshgrid(h,lon)
      cloud_mass = np.swapaxes(cloud_mass,0,2)
      cs = plt.contourf(lons[x1:x2,:61],height[x1:x2,:61],cloud_mass[:,100,:],level= level)
      plt.colorbar(cs,extend='both')
      plt.show() 
'''
      WP = LWP+IWP
      WP[WP<4e-2]=0
      [mask,n_objects] = scipy.ndimage.measuroud_massments.label(WP)
    ...
    out_file = data_path+'netcdf_summary_files/aircraft_comparisons/' + VARIABLE_NAME +'_cell_size_'+str(LOWER_SIZE)+'_to_'+str(UPPER_SIZE)+'_3D_WP_less_than_1eminus1_cloud_mass_less_than_1eminus6.nc'
