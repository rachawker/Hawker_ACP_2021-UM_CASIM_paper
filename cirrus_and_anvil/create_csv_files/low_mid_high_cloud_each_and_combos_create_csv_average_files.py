...
csv_name = 'Average_CLOUD_FRACTION_each_and_combos_low_mid_high_cloud.py'
array = np.zeros((10,7))
...
print array
ra.cloud_cover_each_and_combos_write_csv(array,csv_name)
'''
...
fig.tight_layout()
fig_name = fig_dir + 'low_mid_high_cloud_each_and_combos_stacked_plot.png'
...
csv_name = 'HM_minus_noHM_CLOUD_COVER_CHANGE_each_and_combinations_low_mid_high_cloud'
array = np.zeros((5,7))
fig = plt.figure(figsize=(7,10))
axcd = plt.subplot2grid((4,1),(0,0))
axmt = plt.subplot2grid((4,1),(1,0))
...
for f in range(0,len(list_of_dir)):
  ...
  for n in range(0, len(cloud_fracs)):
    ...
    #ax.set_ylabel(y_labels[n])
print array
ra.cloud_cover_HM_diff_each_and_combos_write_csv(array,csv_name)
'''
handles, labels = axT.get_legend_handles_labels()
display = (0,1,2,3,4)
#simArtist = plt.Line2D((0,1),(0,0), color='k', linewidth=1.2, linestyle='-')
