...
def separate_radiation_cloud_fraction_by_levels_including_combos(data_path):
    ...
    for t in np.arange(start_time*60,end_time*60-1,dt_output[n]):
      ...
      SW_cloudy = copy.deepcopy(shortwave)
      ...
      SW_no_cloud = np.nanmean(SW_no_cloud)
      SW_low = np.nanmean(SW_low)
      SW_low_mid = np.nanmean(SW_low_mid)
      SW_low_mid_high = np.nanmean(SW_low_mid_high)
      SW_low_high = np.nanmean(SW_low_high)
      SW_mid = np.nanmean(SW_mid)
      SW_mid_high = np.nanmean(SW_mid_high)
      SW_high = np.nanmean(SW_high)
      SW_total = np.nanmean(shortwave)
      SW_cloudy = np.nanmean(SW_cloudy)
      ...
      SW_cloudy_array.append(SW_cloudy)

    variables = [LW_no_cloud_array,
      LW_low_array,
      LW_low_mid_array,
      LW_low_mid_high_array,
      LW_low_high_array,
    ...
    out_file = data_path+'netcdf_summary_files/cirrus_and_anvil/LW_and_SW_includes_total_and_cloudy_sep_by_cloud_fraction_each_and_combinations.nc'
