...
for n in range(0,5):
  fig, (ax1,ax2,ax3) = plt.subplots(1,3,subplot_kw={'projection': ccrs.PlateCarree()}, figsize=(15,5),sharey=True)
  #ax = axes[n]
  print n
  ...
  for r in range(0,3):
    ax=axes[r]
    if r == 0:
      out_file = '/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/ICED_CASIM_b933_time_series/BEN_AGU/cloud_type_2D_fileHOMOG_and_HM_no_het__'+date+'.nc'
    if r==1:
  ...
  ax3.set_title(titles[n]+': INP, no HM')
  bels = ['Low','Low/mid','Low/mid/high','Low/high','Mid','Mid/high','High']
  fig.subplots_adjust(right=0.8)
  x=[]
  for i in range(0,7):
    patch = mpatches.Patch(color=cols[i], label=bels[i])
    x.append(patch)
  ax3.legend(handles=x,bbox_to_anchor = (1.05,-0.2),loc='lower left')#, fontsize=9)
  name = names[n]
  print 'Save figure'
  fig_name = fig_dir + name+'_3_panel_cloud_type'+date+'.png'
