...
'''
for f in range(0,len(param_dir)):
    data_path = rl.HM_dir[f]
    label = rl.labels[f]
    line = '--'
    col = rl.col[f]
    name = rl.name[f]
    ...
    n, bins, _ = ax.hist(cth, num_bins, normed=1, histtype='step',color='white')
    bin_centers = 0.5*(bins[1:]+bins[:-1])
    ax.plot(bin_centers,n,c=col,linewidth=1.2, linestyle=line)
'''
...
#fig.tight_layout()
fig_name = fig_dir + 'CTH_distribution.png'
...
for f in range(0,len(param_dir)):
    ...
    input1 = data_path + 'netcdf_summary_files/cirrus_and_anvil/' + file1
    #cbh = ra.read_in_nc_variables(input1,'CBH')
    cth = ra.read_in_nc_variables(input1,'CBH')
    num_bins = 20
    n, bins, _ = ax.hist(cth, num_bins, normed=1, histtype='step',color='white')
    bin_centers = 0.5*(bins[1:]+bins[:-1])
    ax.plot(bin_centers,n,c=col,linewidth=1.2, linestyle=line)
...
#,bbox_to_anchor=(1, -0.4), fontsize=5)
#ax.set_xlim(6,24)
ax.set_xlabel('Cloud base height (km)')
ax.set_ylabel('Normalised frequency')
#fig.tight_layout()
fig_name = fig_dir + 'CBH_distribution.png'
plt.savefig(fig_name, format='png', dpi=1000)
plt.show()
