...
fig_dir = '/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/ICED_CASIM_b933_time_series/FOR_PAPER/NAT_COMMS_FINAL_DRAFT/'#rl.cirrus_and_anvil_fig_dir
...
for x in range(0,len(param_names)):
  ...
  for l in range(0,len(list(bars))):
    if bars[l]>1:    
      axa.text(r[l],-0.1,str(round(bars[l],2)),verticalalignment='top',rotation=90)
    else:
      axa.text(r[l],0.1,str(round(bars[l],2)),verticalalignment='bottom',rotation=90)
y = (1,2,3,4,5,6,7)
print column_names
...
#axb.set_title('(a.) Shortwave radiation')
axa.set_title('(b.) Total outgoing radiation: SIP impact')
axc.set_title('(a.) Total outgoing radiation: INP impact')
...
for x in range(0,len(param_names)):
  ...
  for l in range(0,len(list(bars))):
    if bars[l]>1:
      axc.text(r[l],-0.1,str(round(bars[l],2)),verticalalignment='top',rotation=90)
    else:
      axc.text(r[l],0.1,str(round(bars[l],2)),verticalalignment='bottom',rotation=90)
y = (1,2,3,4,5,6,7)
print column_names
...
axhom.set_title('(c.) Domain mean outgoing radiation')#, fontsize=8)
...
fig_name = fig_dir + 'FOR_PAPER_BAR_CHART_INP_and_HM_total_radiation_difference_and_TOTRADFIGSLOPE_add_value_of_bar_as_text.png'
fig.tight_layout()
