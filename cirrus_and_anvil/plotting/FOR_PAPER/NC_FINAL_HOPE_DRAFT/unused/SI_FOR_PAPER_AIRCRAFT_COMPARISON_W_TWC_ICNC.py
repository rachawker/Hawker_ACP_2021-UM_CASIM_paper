...
#air_CDNC = ra.read_in_nc_variables(air_CDNC_file, air_CDNC_var)
...
make_2D_hist_overlay_aircraft(ax2,updrafts,TWC,air_up,air_TWC,'(b.) Vertical wind and TWC','w '+rl.m_per_s,'TWC '+rl.g_per_m_cubed)#,fig_dir,'2D_hist_with_aircraft_data_W_v_TWC_3gkg_limit')
ax2.set_ylim(0,1600)
...
def make_2D_hist_overlay_aircraft(ax,x,y,air_x,air_y,PLOT_TITLE,x_label,y_label):#,fig_dir,figname):
    ...
    #cmap.set_over('k')
    cmap.set_under('w')
    cb =ax.hexbin(x,y,mincnt=1,bins='log',yscale='log',cmap=cmap,linewidths=5)
    ax.plot(air_x,air_y,'o',markerfacecolor='none', markeredgecolor='k')#,alpha=0.5)
    #plt.hexbin(x,y,cmap=cmap)
    #plt.axis([xmin, xmax, ymin, ymax])
    ax.set_title(PLOT_TITLE)
    fig.subplots_adjust(right=0.8)
    cax =fig.add_axes([0.81, 0.05, 0.05, 0.25])
    cx =fig.colorbar(cb, cax=cax, orientation='vertical')
    #ax.axis([xmin, xmax, ymin, ymax])
    ax.set_title(PLOT_TITLE+ ' (in cloud)',fontsize=9)
    #cb = ax.set_colorbar()
...
make_2D_hist_overlay_aircraft(ax3,ALT,ICNC,air_alt,aICNC,'(c.) Altitude and ICNC','Altitude (km)','ICNC'+rl.per_L)


fig.tight_layout()
fig.subplots_adjust(right=0.8)
fig_name = fig_dir + 'SI_FOR_PAPER_NAT_COMMS_AIRCRAFT_COMPARISON_include_ICNC.png'
