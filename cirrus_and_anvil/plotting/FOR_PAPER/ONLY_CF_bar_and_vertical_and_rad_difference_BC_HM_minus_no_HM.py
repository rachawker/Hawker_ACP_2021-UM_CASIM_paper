...
#cloud fraction
ax1 = plt.subplot2grid((3,5),(0,0),rowspan=3,colspan=2)#(2,3),(0,0),rowspan=2)
#cloud fraction change barchart
#axb = plt.subplot2grid((5,6),(2,3),rowspan=2,colspan=2)#(2,3),(0,1))#,colspan=2)
#ice crystal number conc
#axa = plt.subplot2grid((5,6),(0,2),rowspan=3,colspan=2)#(2,3),(1,1))
#homog freezing
#axc = plt.subplot2grid((5,6),(0,4),rowspan=3,colspan=2)
#total outgoing rad
axd = plt.subplot2grid((3,5),(0,2),rowspan=2,colspan=3)
axf = plt.subplot2grid((3,5),(2,2),rowspan=1,colspan=2)
...
axhom.set_xlabel('Percentage change (%)', fontsize=9, labelpad=10)
axhom.set_ylim(0,16)
axhom.set_xlim(-50,20)
axhom.set_title('(b.) Cloud cover')
#axhom.legend(loc='lower left', fontsize=9)
'''
####ICNC diff
cloud_mass = ['ice_crystal_number','homogeneous_freezing_rate','snow_mmr']

cloud_file = ['/um/netcdf_summary_files/in_cloud_profiles/hydrometeor_number_profiles.nc','/um/netcdf_summary_files/in_cloud_profiles/freezing_rate_profiles.nc','/um/netcdf_summary_files/in_cloud_profiles/cloud_mass_profiles.nc']

mass_title =['ICNC','Homogeneous freezing','Ice effective radius','Snow + graupel mass','Ice/snow mass ratio', 'Ice/graupel mass ratio' ]
xmass_title =['Number concentration\n','Ice particle production\n']
mass_name=['Ice_crystal_number','homogeneous_freezing_rate','snow_graupel_mass']
units = [rl.per_m_cubed_div_by_10000,rl.per_m_cubed_per_s_div_by_1000,rl.um,rl.kg_per_m_cubed_by_minus_10000,'','']

list_of_dir = ['/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/Cooper1986','/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/Meyers1992','/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/DeMott2010','/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/Niemand2012', '/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/Atkinson2013','/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/HOMOG_and_HM_no_het']

HM_di =['/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/NO_HM_Cooper1986', '/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/NO_HM_Meyers1992', '/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/NO_HM_DM10','/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/NO_HM_Niemand2012','/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/NO_HM_Atkinson2013']
...
for f in range(0,len(list_of_dir)-1):
  ...
  for n in range(0,2):
    ...
    zeros = np.zeros(len(height))
    axone.set_xlabel(r'$\Delta$'+ ' '+xmass_title[n]+units[n], fontsize=9)#, labelpad=16)
    #axone.text(0.15,0.25,ax_lab1[n],transform=axone.transAxes,fontsize=9)
    zeros = np.zeros(len(height))
    axone.minorticks_off()
    axone.plot(zeros,height,c='k',linestyle='-',zorder=0)
    axone.set_title(axlab[n]+' ' +mass_title[n])
    #axone.locator_params(nbins=5, axis='x')
    #axone.xaxis.set_major_formatter(FormatStrFormatter('%.1e'))
    axone.xaxis.get_major_formatter().set_powerlimits((0, 1))
    axone.locator_params(nbins=5, axis='x')
    axone.ticklabel_format(axis='x', style='plain', scilimits=(0,0))
handles, labels = ax1.get_legend_handles_labels()
display = (0,2,4,6,8)
axone.legend(handles,labels,loc='lower left', fontsize=9)
'''
####RADIATION
...
axf.legend(handles=x,loc='upper left',fontsize=9)
#axd.legend(handles=x,bbox_to_anchor=(1.3,0.1),loc='lower left')
axf.axis('off')
axd.set_title('(a.) Total outgoing radiation')



fig_name = fig_dir+'/only_CF_BC_and_vertical_and_rad_BC_HM_minus_noHM_diff.png'
