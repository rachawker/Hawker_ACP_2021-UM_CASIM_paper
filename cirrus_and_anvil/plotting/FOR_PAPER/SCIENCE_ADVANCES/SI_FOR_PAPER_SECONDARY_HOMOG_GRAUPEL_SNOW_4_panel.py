...
for f in range(0,len(list_of_dir)):
  ...
  for n in range(0,2):
    ...
    if n==0:
      axone.xaxis.set_major_locator(plt.MaxNLocator(5))
      axone.set_xlim(0,1800)
...
xmass_title =[rl.capC+' Ice crystal number\n concentration',rl.capD+' Cloud droplet mass\n concentration','(e.) Snow mass\n','(f.) Graupel mass\n' ]
...
mass_title =[rl.capC+' ICNC',rl.capD+' Cloud droplet mass',rl.capC+' Snow mass',rl.capD+' Graupel mass' ]
...
###ax5 = plt.subplot2grid((2,2),(1,1))#,sharey=ax1)
...
fig.tight_layout()
fig_name = fig_dir + 'SI_PARAM_NEW_4_panel.png'
