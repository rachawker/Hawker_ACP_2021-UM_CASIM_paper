...
fig, (ax1, ax2, ax3) = plt.subplots(ncols=3, subplot_kw={'projection': ccrs.PlateCarree()}, figsize = (7.8,4))
  ###PLOT MODEL DATA
level = np.arange(0,900,50)
cmap = cmx.get_cmap('Greys_r')
norm = BoundaryNorm(level, ncolors=cmap.N, clip=False)

cs=ax1.pcolormesh(lon,lat,dbz,cmap=cmap,norm=norm,transform=ccrs.PlateCarree())
fig.subplots_adjust(right=0.8)
cbar_ax = fig.add_axes([0.875, 0.33, 0.015, 0.3])  #left, bottom, width, height as fractions of figure dimensions
cbar =  fig.colorbar(cs, cax=cbar_ax)
cbar.set_label("(a.) Outgoing TOA \nshortwave" + rl.W_m_Sq)
...
for n in range (0,2):
  ...
  plt.xticks([])
  plt.yticks([])
  ax.text(0.6,0.8,labels[n],transform=ax.transAxes, color='k')#,bbox=dict(facecolor='white'))
...
print 'Save figure'
fig_name = fig_dir + 'High_cloud_2D_difference'+date+'.png'
