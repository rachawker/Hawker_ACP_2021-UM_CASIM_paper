...
axb.set_ylabel(r'$\Delta$'+ ' SW \n(albedo) '+rl.W_m_Sq)#, fontweight='bold')
...
axa.set_ylabel(r'$\Delta$'+ ' LW \n(albedo)  '+rl.W_m_Sq)

fig_name = fig_dir + 'BAR_CHART_cloud_albedo_radiation_difference_HM_minus_noHM_diff.png'
fig.tight_layout()
...
axb.tick_params(axis='x',          # changes apply to the x-axis
    ...
    labelbottom=False)
#plt.setp(axb.get_xticks(), visible=False)
axb.set_ylabel(r'$\Delta$'+ ' SW \n(domain) '+rl.W_m_Sq)#, fontweight='bold')
...
for i in range(0,5):
    patch = mpatches.Patch(color=rl.col[i], label=bels[i])
    x.append(patch)
#axa.legend(handles=x,bbox_to_anchor = (0.08,0.05),loc='lower left', fontsize=9,ncol=2)

axa.set_ylabel(r'$\Delta$'+ ' LW \n(domain) '+rl.W_m_Sq)

fig_name = fig_dir + 'BAR_CHART_Domain_integrated_radiation_difference_HM_minus_noHM_diff.png'
fig.tight_layout()
plt.savefig(fig_name, format='png', dpi=500)
plt.show()
