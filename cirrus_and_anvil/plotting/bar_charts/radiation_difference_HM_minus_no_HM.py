...
file_location = '/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/ICED_CASIM_b933_time_series/cirrus_and_anvil/csv_files/radiation/'

#domain_diff_file = 'LW_and_SW_sep_by_cloud_fraction_each_and_combinations.nc'

####MEAN difference W/m2 for each cloud type is in HM_minus_noHM_LW_SW_mean_radiation_DIFFERENCE_for_each_and_combinations_low_mid_high_cloud.csv

####TOTAL difference (total domain outgoing radiation) is in HM_minus_noHM_LW_SW_TOTAL_radiation_DIFFERENCE_for_each_and_combinations_low_mid_high_cloud.csv

domain_diff_file = 'HM_minus_noHM_LW_SW_mean_radiation_DIFFERENCE_for_each_and_combinations_low_mid_high_cloud.csv'
...
plt.ylabel('Mean Difference TOA outgoing longwave (HM - no HM) (W m2)', fontweight='bold')

fig_name = fig_dir + 'bar_charts/BAR_CHART_cloud_albedo_longwave_difference_HM_minus_noHM_diff.png'
fig.tight_layout()
...
plt.xlabel('Cloud level', fontweight='bold')
plt.xticks([r + ((5*barWidth)/2) for r in range(len(bars))], column_names)
plt.ylabel('Mean Difference TOA outgoing shortwave (HM - no HM) (W m2)', fontweight='bold')

fig_name = fig_dir + 'bar_charts/BAR_CHART_cloud_albedo_shortwave_difference_HM_minus_noHM_diff.png'
fig.tight_layout()
plt.savefig(fig_name, format='png', dpi=500)
plt.show()

####Domain integrated change
domain_diff_file = 'HM_minus_noHM_LW_SW_TOTAL_radiation_DIFFERENCE_for_each_and_combinations_low_mid_high_cloud.csv'
domain_diff = file_location+domain_diff_file
domain_diff_data = np.genfromtxt(domain_diff, delimiter =',')
fig_dir = rl.cirrus_and_anvil_fig_dir
...
plt.xlabel('Cloud level', fontweight='bold')
plt.xticks([r + ((5*barWidth)/2) for r in range(len(bars))], column_names)
plt.ylabel('TOA outgoing longwave (HM - no HM) (W m2)', fontweight='bold')

fig_name = fig_dir + 'bar_charts/BAR_CHART_domain_integrated_longwave_difference_HM_minus_noHM_diff.png'
fig.tight_layout()
plt.savefig(fig_name, format='png', dpi=500)
plt.show()
...
plt.xlabel('Cloud level', fontweight='bold')
plt.xticks([r + ((5*barWidth)/2) for r in range(len(bars))], column_names)
plt.ylabel('TOA outgoing shortwave (HM - no HM) (W m2)', fontweight='bold')

fig_name = fig_dir + 'bar_charts/BAR_CHART_Domain_integrated_shortwave_difference_HM_minus_noHM_diff.png'
fig.tight_layout()
plt.savefig(fig_name, format='png', dpi=500)
plt.show()
'''
...
plt.xlabel('Cloud level', fontweight='bold')
plt.xticks([r + ((5*barWidth)/2) for r in range(len(bars))], column_names)
plt.ylabel('Percentage change in cloudy fraction (HM - no HM) (%)', fontweight='bold')

fig_name = fig_dir + 'bar_charts/BAR_CHART_cloud_cover_cloudy_percentage_change_HM_minus_noHM_diff.png'
fig.tight_layout()
plt.savefig(fig_name, format='png', dpi=500)
plt.show()
'''
