...
cloud_mass =['convective_cell_number','mean_convective_cell_size','max_convective_cell_size','min_convective_cell_size']

number = ['ice_crystal_number',
      'snow_number',
      'graupel_number']

mass_name =['convective_cell_number','mean_convective_cell_size','max_convective_cell_size','min_convective_cell_size']
...
files = ['cloud_cell_number_by_height_cloud_lim_10eminus5.nc','cloud_cell_number_by_height_cloud_lim_10eminus6.nc','cloud_cell_number_by_height_cloud_lim_10eminus7.nc']
...
for n in range(0,4):
  ...
  for f in range(0,6):
   ...
   for x in range(0,3):
    ...
    print data_path
    file1=data_path+'/um/netcdf_summary_files/cirrus_and_anvil/'+files[x]#cloud_cell_number_by_height.nc'
    ...
    if x==1:
     ...
     axhom.plot(M,height,c=col[f], linewidth=1.2, linestyle='-.',label=labels[f])
    else:
     axhom.plot(M,height,c=col[f], linewidth=1.2, linestyle='--',label=labels[f])
  axhom.set_ylabel('Height (km)', fontsize=9)
  axhom.set_xlabel(mass_name[n], fontsize=9, labelpad=10)
  axhom.set_ylim(0,16)
  #axhom.ticklabel_format(format='%.0e',axis='x')
  #axhom.set_title(mass_name[r], fontsize=8)
  fig.tight_layout()
  fig_name = fig_dir +mass_name[n]+'_mean_profile_check_cloud_limits.png'
