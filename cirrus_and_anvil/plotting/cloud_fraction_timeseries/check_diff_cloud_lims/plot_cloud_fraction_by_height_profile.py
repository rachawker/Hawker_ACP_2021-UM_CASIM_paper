...
files = ['cloud_fraction_by_height_cloud_lim_10eminus5.nc','cloud_fraction_by_height_cloud_lim_10eminus6.nc','cloud_fraction_by_height_cloud_lim_10eminus7.nc']
...
for f in range(0,6):
  for x in range(0,3):
    ...
    print data_path
    name_of_file = files[x]
    file1=data_path+'/um/netcdf_summary_files/cirrus_and_anvil/'+files[x]#cloud_fraction_by_height.nc'
    ...
    ice_mass_mean = np.nanmean(ice_mass[24:,:],axis=0)
...
fig.tight_layout()
fig_name = fig_dir +'cloud_fraction_mean_profile_check_cloud_limits.png'
...
for f in range(0,(len(list_of_dir))):
  for x in range(0,3):
    ...
    if x==1:
      ...
      axhom.plot(fr1,height,c=col[f], linewidth=1.2, linestyle='-.')
    else:
      axhom.plot(fr1,height,c=col[f], linewidth=1.2, linestyle='--')
    zeros = np.zeros(len(height))
    axhom.plot(zeros,height,c='k')
axhom.set_xlim(-60,50)
axhom.set_ylabel('Height (km)', fontsize=8)
axhom.set_xlabel(r'$\Delta$'+ ' Cloud fraction % change', fontsize=8, labelpad=10)
axhom.set_ylim(0,16)
#axhom.ticklabel_format(format='%.0e',axis='x')
#axhom.set_title(mass_name[r], fontsize=8)#fig.tight_layout()
fig_name = fig_dir + 'cloud_fraction_mean_Het_minus_noHet_percentage_diff_profiles_check_cloud_limits.png'
plt.savefig(fig_name, format='png', dpi=400)
plt.show()
# plt.close()
