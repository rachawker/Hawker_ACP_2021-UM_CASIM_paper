...
for n in range(0,4):
  ...
  fig.tight_layout()
  fig_name = fig_dir +mass_name[n]+'_mean_profile.png'
...
for n in range(0,4):
  ...
  axhom.set_xlabel(r'$\Delta$ '+ mass_name[n], fontsize=8, labelpad=10)
  axhom.set_ylim(0,16)
#axhom.ticklabel_format(format='%.0e',axis='x')
#axhom.set_title(mass_name[r], fontsize=8)#fig.tight_layout()
  fig_name = fig_dir + mass_name[n]+'_HM_minus_noHM_diff_profiles.png'
  plt.savefig(fig_name, format='png', dpi=400)
  #plt.show()
  plt.close()
...
for n in range(0,4):
  ...
  for f in range(0,(len(list_of_dir))-1):
    ...
    zeros = np.zeros(len(height))
    axhom.plot(zeros,height,c='k')
  axhom.set_xlim(-75,75)
  #axhom.set_ylabel('Height (km)', fontsize=8)
  axhom.set_xlabel('Percentage change '+ mass_name[n], fontsize=8, labelpad=10)
  axhom.set_ylim(0,16)
  fig_name = fig_dir +mass_name[n]+'_mean_percentage_change_HM_minus_noHM_profile.png'
  plt.savefig(fig_name, format='png', dpi=400)
  plt.show()
