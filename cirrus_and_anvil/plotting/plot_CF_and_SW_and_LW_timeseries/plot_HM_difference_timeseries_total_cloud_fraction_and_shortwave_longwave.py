  ...
  for n in range(0, len(axes)):
    if n==0:
      for i in range(0, len(cloud_fracs)):
        ...
        hfile = HM_data_path+cloud_fracs_file
        cfi2 = ra.read_in_nc_variables(hfile,cloud_fracs[i])
        cfi = cfi-cfi2
        if i==0:
          cf = cfi
        else:
      ...
      hfile = HM_data_path+'/netcdf_summary_files/TOA_outgoing_radiation_timeseries.nc'
      nc = netCDF4.Dataset(rfile)
      cf = nc.variables[rads[n-1]]
      nc2 = netCDF4.Dataset(hfile)
      cf2 = nc2.variables[rads[n-1]]
      if n==1:
        cf = cf[40:]
        cf2 = cf2[40:]
        cf = cf-cf2
...
#fig.tight_layout()
fig_name = fig_dir + 'HM_difference_cloud_fraction_shortwave_longwave_timeseries.png'
fig_name = fig_dir + 'legend_just_params.png'
