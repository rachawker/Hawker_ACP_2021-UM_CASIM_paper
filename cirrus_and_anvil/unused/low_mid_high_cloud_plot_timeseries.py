...
cloud_fracs_file = rl.cloud_fracs_file
cloud_fracs = rl.cloud_fracs
...
axcd = plt.subplot2grid((5,1),(0,0))
axmt = plt.subplot2grid((5,1),(1,0))
axg = plt.subplot2grid((5,1),(2,0))
axT = plt.subplot2grid((5,1),(3,0))

axes = [axcd,axmt,axg,axT]
y_labels = ['Low cloud', 'Mid cloud', 'High cloud', 'Total cloud']
...
fig.tight_layout()
fig_name = fig_dir + 'low_mid_high_cloud_fraction.png'
plt.savefig(fig_name, format='png', dpi=1000)
plt.show()
   


### Percentage difference HM-noHM low,med and high cloud, all cloud on each level

csv_name = 'HM_minus_noHM_CLOUD_COVER_CHANGE_all_low_mid_high_cloud'
array = np.zeros((5,4))
fig = plt.figure(figsize=(7,10))
axcd = plt.subplot2grid((5,1),(0,0))
axmt = plt.subplot2grid((5,1),(1,0))
...
for f in range(0,len(list_of_dir)):
  ...
  for n in range(0, len(cloud_fracs)):
    ...
    ax = axes[n]
    if n==2:
      ax.plot(time, cf,c=col[f],linewidth=1.2, linestyle=line[f],label=rl.labels[f])
    else:
      ax.plot(time, cf,c=col[f],linewidth=1.2, linestyle=line[f])
...
axT.set_xlabel('Time (hr)')
fig.tight_layout()
fig_name = fig_dir + 'low_mid_high_cloud_fraction_Param_minus_HM.png'
plt.savefig(fig_name, format='png', dpi=1000)
plt.show() 
