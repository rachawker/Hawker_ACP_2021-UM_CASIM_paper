...
units = [rl.per_m_cubed,rl.kg_per_m_cubed_by_minus_10000,rl.um,rl.kg_per_m_cubed_by_minus_10000,'','']
...
for f in range(0,len(list_of_dir)-1):
  ...
  for n in range(0,6):
    if n==2:
      ...
      file1=HM_dir+'/um/netcdf_summary_files/in_cloud_profiles/cloud_mass_profiles.nc'
      ice_mass = ra.read_in_nc_variables(file1,cloud_mass[1])
      file2 = HM_dir+'/um/netcdf_summary_files/in_cloud_profiles/hydrometeor_number_profiles.nc'
      ice_no = ra.read_in_nc_variables(file2,cloud_mass[0])
      ice_no = ice_no[24:,:]
      ice_mass = ice_mass[24:,:]
      ice_mass_mean = np.nanmean(ice_mass,axis=0)
    ...
    if n==4 or n==5:
      axone.set_xscale('symlog')#,linthreshx=10e1)
      axone.set_xlim(-10e1,0)
    if n==2:
      axone.set_xlim(-120,100)
...
display = (0,2,4,6,8)
ax6.legend([handle for i,handle in enumerate(handles) if i in display], [label for i,label in enumerate(labels) if i in display],loc='upper right')#bbox_to_anchor=(0.7, 0.85), fontsize=7.5)
...
fig.tight_layout()
fig_name = fig_dir + 'HM_diff_ice_number_mass_snow_graupel_mass_profiles_6_panel.png'
