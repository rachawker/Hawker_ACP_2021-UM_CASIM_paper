...
def cloud_mass_write_nc(data_path):
    ...
    for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
      ...
      air_density[cloud_mass<10e-6]=np.nan

 
      air_density_mmr = np.nanmean(air_density, axis=(1,2))

      air_density_array.append(air_density_mmr)
    ...
    variables = [air_density_array]

    names = ['air_density']

    out_file = data_path+'netcdf_summary_files/in_cloud_profiles/air_density_in_cloud_profiles.nc'
