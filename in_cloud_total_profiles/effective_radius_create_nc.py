...
def hydrometeor_number_write_nc(data_path):
    ...
    for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
      ...
      ice_crystal_number = calc_effective_radius(ice_crystal_number,ice_crystal_mmr,p1l[0],p2l[0],mul[0],c_xl[0])
      snow_number = calc_effective_radius(snow_number,snow_mmr,p1l[1],p2l[1],mul[1],c_xl[1])
      graupel_number = calc_effective_radius(graupel_number,graupel_mmr,p1l[2],p2l[2],mul[2],c_xl[2])
      CD_number = calc_effective_radius(CD_number,CD_mmr,p1l[3],p2l[3],mul[3],c_xl[3])
      rain_number = calc_effective_radius(rain_number,rain_mmr,p1l[4],p2l[4],mul[4],c_xl[4])

      ice_crystal_number = ice_crystal_number * 1e6
      snow_number = snow_number * 1e6
      graupel_number = graupel_number * 1e6
      CD_number = CD_number * 1e6
      rain_number = rain_number * 1e6
    ...
    names = ['ice_crystal_radius',
      'snow_radius',
      'graupel_radius',
      'CD_radius',
      'rain_radius']

    out_file = data_path+'netcdf_summary_files/in_cloud_profiles/effective_radius_profiles.nc'
