...
def hydrometeor_number_write_nc(data_path):
    ...
    for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
      ...
      ice_crystal_number[high==0]=np.nan
      snow_number[high==0]=np.nan
      graupel_number[high==0]=np.nan
      ice_water_number[high==0]=np.nan
      CD_number[high==0]=np.nan
      rain_number[high==0]=np.nan 
    ...
    out_file = data_path+'netcdf_summary_files/in_cloud_profiles/one_cloud_level_only/high_cloud_only/hydrometeor_number_profiles.nc'
