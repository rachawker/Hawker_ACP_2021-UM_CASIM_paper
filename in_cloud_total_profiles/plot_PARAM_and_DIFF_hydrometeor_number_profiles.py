...
cloud_mass = ['ice_crystal_number',
      'snow_number',
      'graupel_number',
      'CD_number',
      'rain_number']

mass_title = ['ice crystal number',
      'snow number',
      'graupel number',
      'CD number',
      'rain number']
mass_name = ['ice_crystal_number',
      'snow_number',
      'graupel_number',
      'CD_number',
      'rain_number']
...
for r in range(0, len(cloud_mass)):
  ...
  axhom.set_xlabel(mass_title[r]+r" ($\mathrm{m{{^-}{^3}}}$)", fontsize=8, labelpad=10)
...
for r in range(0, len(cloud_mass)):
  ...
  if r == 0:
    axhom.set_xlim(1e1,1e7)
  if r==1:
    axhom.set_xlim(1e1,13000)
  if r == 2:
    axhom.set_xlim(1e0,1e3)
  if r == 3:
    axhom.set_xlim(1e0,1e8)
  if r ==4:
    axhom.set_xlim(1e-1,1e5)
...
for r in range(0, len(cloud_mass)):
  ...
  #fig.tight_layout()
  fig.subplots_adjust()
  fig_name = fig_dir + 'PARAM_ONLY_DIFFERENCE_HM_minus_noHM_'+mass_name[r] + '_LOG_SCALE_in_cloud_mean_profiles.png'
  plt.savefig(fig_name, format='png', dpi=400)
  plt.show()
  #plt.close()
