...
for r in range(0, len(cloud_mass)):
  ...
  for f in range(0,len(list_of_dir)):
    ...
    print data_path
    file1=data_path+'/um/netcdf_summary_files/in_cloud_profiles/updraft_in_cloud_profiles.nc'
...
for r in range(0, len(cloud_mass)):
  ...
  #fig.tight_layout()
  fig_name = fig_dir + 'PARAM_ONLY_DIFFERENCE_HM_minus_noHM_'+mass_name[r] + '_in_cloud_profiles.png'
  plt.savefig(fig_name, format='png', dpi=400)
  plt.show()
...
for r in range(0, len(cloud_mass)):
  ...
  axhom.set_xscale('log')
    ...
    #axhom.set_xlim(-5e-5,5e-5)
  #axhom.locator_params(nbins=5, axis='x')
  axhom.set_xlabel(mass_title[r]+" (% change)", fontsize=8, labelpad=10)
  axhom.set_title('Difference HM minus no HM', fontsize=8)
  fig_name = fig_dir + 'PARAM_ONLY_DIFFERENCE_HM_minus_noHM_LOG_SCALE_percentage_'+mass_name[r] + '_in_cloud_profiles.png'
  plt.savefig(fig_name, format='png', dpi=400)
  plt.show()
