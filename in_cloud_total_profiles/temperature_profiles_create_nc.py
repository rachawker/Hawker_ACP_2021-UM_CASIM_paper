...
def water_vapor_write_nc(data_path):
    ...
    for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
      ...
      rh = np.nanmean(temperature, axis=(1,2))
      qv = np.nanmean(potential_temperature, axis=(1,2))
    ...
    out_file = data_path+'netcdf_summary_files/in_cloud_profiles/temperature_profiles.nc'
