...
def water_vapor_write_nc(data_path):
    ...
    for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
      ...
      Rd_cp = Rd/cp
      air_pressure = Exner**(1/Rd_cp)*p0 #in pascales
      #print 'air_pressure'
      #print air_pressure
      R_specific=287.058
      air_pascales = Exner**(1/Rd_cp)*p0_pascales
      air_density=(air_pascales/(temperature*R_specific))
      #print 'air density'
      #print air_density
      ...
      qv = ra.load_and_limit_cube_zxy(input_file,'m01s00i010',z1,z2,x1,x2,y1,y2)
      mass_per_m_cubed = qv*air_density   #water vapor kg/m^3
      ...
      QSA4=6.109
      TK0=273.15
      Qsat = QSA1/(p*np.exp(QSA2*(T-TK0)/(T-QSA3))-QSA4)
      #print 'RH'
      rh = (qv/Qsat)*100
      ...
      print np.amax(qv)
      print np.nanmax(qv)
      mass_per_m_cubed[cloud_mass>10e-6]=np.nan
      print np.amax(mass_per_m_cubed)
      print np.nanmax(mass_per_m_cubed)

      rh = np.nanmean(rh, axis=(1,2))
      qv = np.nanmean(qv, axis=(1,2))
      mass_per_m_cubed = np.nanmean(mass_per_m_cubed, axis=(1,2))      

      rh_array.append(rh)
      print rh.shape
      qv_array.append(qv)
      mass_per_m_cubed_array.append(mass_per_m_cubed_array)
    ...
    variables = [qv_array, mass_per_m_cubed_array, rh_array]
    #variables = [qv_array,rh_array]

    names = ['Specific_humidity',
      'Mass_of_water_vapor_per_metre_cubed',
      'relative_humidity']
    #names = ['Specific_humidity','relative_humidity']
    #return variables

    out_file = data_path+'netcdf_summary_files/in_cloud_profiles/water_vapor_with_kg_m_cubed_WP_profiles.nc'
