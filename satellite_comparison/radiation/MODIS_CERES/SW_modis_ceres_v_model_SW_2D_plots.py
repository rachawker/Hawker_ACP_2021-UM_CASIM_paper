...
for a in range(0,1):
      ...
      level = np.linspace(np.nanmin(sw_modelt),np.nanmax(sw_modelt),15)
      norm = BoundaryNorm(level, ncolors=cmap.N, clip=False)
      ax1.pcolormesh(satx,saty,sat_all[491,:,:],cmap=cmap,norm=norm)
      ax1.set_xlim(np.nanmin(lont),np.nanmax(lont))
      ax1.set_ylim(np.nanmin(latt),np.nanmax(latt))
      ...
      ax2.pcolormesh(coarse_pop_density,cmap=cmap,norm=norm)
      plt.show()
