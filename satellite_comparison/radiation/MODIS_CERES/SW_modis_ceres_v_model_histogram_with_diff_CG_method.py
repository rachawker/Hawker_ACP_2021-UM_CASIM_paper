...
sat_file = 'CERES_SSF_Aqua-XTRK_Edition4A_Subset_2015082102-2015082114.nc'
sat_path = '/home/users/rhawker/ICED_CASIM_master_scripts/satellite_comparison/radiation/MODIS_CERES/'
sat_data = sat_path+sat_file
sw_data = ra.read_in_nc_variables_maintain_meta_data(sat_data, 'CERES_SW_TOA_flux___upwards')
...
model_sw_data = []
sat_file = netCDF4.Dataset('/home/users/rhawker/ICED_CASIM_master_scripts/satellite_comparison/radiation/CERES_SYN1deg-1H_Terra-Aqua-MODIS_Ed4A_Subset_20150801-20150930.nc')
sat_all = ra.read_in_nc_variables_maintain_meta_data('/home/users/rhawker/ICED_CASIM_master_scripts/satellite_comparison/radiation/CERES_SYN1deg-1H_Terra-Aqua-MODIS_Ed4A_Subset_20150801-20150930.nc','toa_sw_all_1h')
lat_sat = sat_file.variables['lat'][:]
lon_sat = lon = sat_file.variables['lon'][:]-360
satx,saty = np.meshgrid(lon_sat,lat_sat)

name = 'Niemand2012'
data_path = '/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/Niemand2012/um/'
date = dates[0]
file_name = name+'_pk'+date+'.pp'
input_file = data_path+file_name

dbz_cube=iris.load_cube(input_file,(iris.AttributeConstraint(STASH='m01s02i205')))#m01s01i208')))
print dbz_cube
dbz=dbz_cube.data[:,:]
print np.amax(dbz)
...
for a in range(0,1):
      ...
      sw_modelt = sw_model.flatten()
      gridded = scipy.interpolate.griddata((lont,latt),sw_modelt,(satx,saty),method='cubic')
      #ax2.pcolormesh(satx,saty,gridded,cmap=cmap,norm=norm)
      #ax2.set_xlim(np.nanmin(lont),np.nanmax(lont))
      #ax2.set_ylim(np.nanmin(latt),np.nanmax(latt))
      #plt.show()
      coarseness=111
...
for f in range(0,6):
    ...
    for a in range(0,8):
      ...
      sw_model = ra.load_and_limit_cube_xy(input_file,sw_stash,x1,x2,y1,y2)
      sw_modelt = sw_model.flatten()
      gridded = scipy.interpolate.griddata((lont,latt),sw_modelt,(satx,saty),method='linear')
      coarseness=111
      sw_for_cg_model = sw_model[:555,:]    
      shape = np.array(sw_for_cg_model.shape, dtype=float)
      new_shape = coarseness * np.ceil(shape / coarseness).astype(int)
      ...
      for i in range(770,777):
          zp_pop_density[:, i] = sw_for_cg_model[:,769]
      temp = zp_pop_density.reshape((new_shape[0] // coarseness, coarseness, new_shape[1] // coarseness, coarseness)         )
      coarse_pop_density = gridded#np.mean(temp, axis=(1,3))
      all_model = coarse_pop_density.flatten()
      model_sw_data.append(all_model)

    all_model_sw = np.asarray(model_sw_data)
    all_model_final = all_model_sw.flatten()
    all_model_final = all_model_final[~np.isnan(all_model_final)]#all_model_final[all_model_final!=np.nan]
    num_bins = 10
    #n, bins, _ = ax.hist(all_model_final, num_bins, normed=1, histtype='step',color='white', alpha=1)
...
fig.tight_layout()
plt.legend()
plt.savefig('/group_workspaces/jasmin2/asci/rhawker/ICED_b933_case/ICED_CASIM_b933_time_series/FOR_PAPER/sw_outgoing_model_vs_satellite_regrid_to_satellite_grid.png', format='png', dpi=500)
plt.show()
