...
out_file = data_path+'netcdf_summary_files/LWP_IWP_CD_rain_IC_graupel_snow_in_cloud_only.nc'
...
for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
  ...
  if date == '00000000':
    liquid_water_mmr=liquid_water_mmr[0,:,:,:]
    CD_mmr=CD_mmr[0,:,:,:]
    rain_mmr=rain_mmr[0,:,:,:]
    ice_water_mmr=ice_water_mmr[0,:,:,:]
    snow_mmr=snow_mmr[0,:,:,:]
    graupel_mmr=graupel_mmr[0,:,:,:]
    ice_crystal_mmr=ice_crystal_mmr[0,:,:,:]
  else:
    liquid_water_mmr=liquid_water_mmr
    CD_mmr=CD_mmr
    rain_mmr=rain_mmr
    ice_water_mmr=ice_water_mmr
    snow_mmr=snow_mmr
    graupel_mmr=graupel_mmr
    ice_crystal_mmr=ice_crystal_mmr

  if date == '00000000':
    cloud_mass = liquid_water_mmr.data[:,:,:]+ice_water_mmr.data[:,:,:]
  else:
    cloud_mass = liquid_water_mmr.data[:,:,:]+ice_water_mmr.data[:,:,:]
  cloud_mass[cloud_mass<10e-6]=0

  liquid_water_mmr.data[cloud_mass==0]=np.nan
  CD_mmr.data[cloud_mass==0]=np.nan
  rain_mmr.data[cloud_mass==0]=np.nan

  liquid_water_mc=air_density[0,:,:,:]*liquid_water_mmr
  ...
  CD_mmr.coord('sigma').bounds = None
  CD_mmr.coord('level_height').bounds = None
  CD_mc=air_density[0,:,:,:]**CD_mmr
  CD_column=np.empty(CD_mc.data.shape[0]).tolist()
  CD_column=(CD_mc*length_gridbox_cube)
  CD=CD_column.collapsed(['model_level_number'],iris.analysis.SUM)

  del CD_mmr
  del CD_mc
  del CD_column

  rain_mmr.coord('sigma').bounds = None
  rain_mmr.coord('level_height').bounds = None
  rain_mc=air_density[0,:,:,:]**rain_mmr
  rain_column=np.empty(rain_mc.data.shape[0]).tolist()
  rain_column=(rain_mc*length_gridbox_cube)
  rain=rain_column.collapsed(['model_level_number'],iris.analysis.SUM)
  ...
  ice_crystal_mmr.coord('sigma').bounds = None
  ice_crystal_mmr.coord('level_height').bounds = None
  IC_mc=air_density*ice_crystal_mmr
  IC_column=np.empty(IC_mc.data.shape[0]).tolist()
  IC_column=(IC_mc*length_gridbox_cube)
  IC=IC_column.collapsed(['model_level_number'],iris.analysis.SUM)

  del ice_crystal_mmr
  del IC_mc
  del IC_column

  graupel_mmr.coord('sigma').bounds = None
  graupel_mmr.coord('level_height').bounds = None
  g_mc=air_density*graupel_mmr
  g_column=np.empty(g_mc.data.shape[0]).tolist()
  g_column=(g_mc*length_gridbox_cube)
  g=g_column.collapsed(['model_level_number'],iris.analysis.SUM)

  del graupel_mmr
  del g_mc
  del g_column

  snow_mmr.coord('sigma').bounds = None
  snow_mmr.coord('level_height').bounds = None
  s_mc=air_density*snow_mmr
  s_column=np.empty(s_mc.data.shape[0]).tolist()
  s_column=(s_mc*length_gridbox_cube)
  s=s_column.collapsed(['model_level_number'],iris.analysis.SUM)
  ...
  IWP[cloud_mass==0] = np.nan
  IC[cloud_mass==0] = np.nan
  g[cloud_mass==0] = np.nan
  s[cloud_mass==0] = np.nan
