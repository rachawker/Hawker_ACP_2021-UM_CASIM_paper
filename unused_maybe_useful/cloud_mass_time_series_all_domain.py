...
out_file = data_path+'netcdf_summary_files/cloud_mass_all_domain.nc'
...
for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
  ...
  if date == '00000000':
    cloud_mass = liquid_water_mmr.data[0,:,:,:]+ice_water_mmr.data[0,:,:,:]
    CD_mmr = CD_mmr.data[0,:,:,:]
    rain_mmr = rain_mmr.data[0,:,:,:]
    liquid_water_mmr = liquid_water_mmr.data[0,:,:,:]
    ice_crystal_mmr = ice_crystal_mmr.data[0,:,:,:]
    graupel_mmr = graupel_mmr.data[0,:,:,:]
    snow_mmr = snow_mmr.data[0,:,:,:]
    ice_water_mmr = ice_water_mmr.data[0,:,:,:]
  else:
    cloud_mass = liquid_water_mmr.data[:,:,:]+ice_water_mmr.data[:,:,:]
    CD_mmr = CD_mmr.data
    rain_mmr = rain_mmr.data
    liquid_water_mmr = liquid_water_mmr.data
    ice_crystal_mmr = ice_crystal_mmr.data
    graupel_mmr = graupel_mmr.data
    snow_mmr = snow_mmr.data
    ice_water_mmr = ice_water_mmr.data



  lsum = np.nansum(liquid_water_mmr.data)
  liquid_sum_timeseries.append(lsum)
  lmean = np.nanmean(liquid_water_mmr.data)
  liquid_mean_timeseries.append(lmean)
  lmin = np.nanmin(liquid_water_mmr.data)
  liquid_min_timeseries.append(lmin)
  lmax = np.nanmax(liquid_water_mmr.data)
  liquid_max_timeseries.append(lmax)


  cdsum = np.nansum(CD_mmr.data)
  cloud_drop_mass_sum_timeseries.append(cdsum)
  cdmean = np.nanmean(CD_mmr.data)
  cloud_drop_mass_mean_timeseries.append(cdmean)
  cdmin = np.nanmin(CD_mmr.data)
  cloud_drop_mass_min_timeseries.append(cdmin)
  cdmax = np.nanmax(CD_mmr.data)
  cloud_drop_mass_max_timeseries.append(cdmax)

  rsum = np.nansum(rain_mmr.data)
  rain_mass_sum_timeseries.append(rsum)
  rmean = np.nanmean(rain_mmr.data)
  rain_mass_mean_timeseries.append(rmean)
  rmin = np.nanmin(rain_mmr.data)
  rain_mass_min_timeseries.append(rmin)
  rmax = np.nanmax(rain_mmr.data)
  ...
  isum = np.nansum(ice_water_mmr.data)
  ice_sum_timeseries.append(isum)
  imean = np.nanmean(ice_water_mmr.data)
  ice_mean_timeseries.append(imean)
  imin = np.nanmin(ice_water_mmr.data)
  ice_min_timeseries.append(imin)
  imax = np.nanmax(ice_water_mmr.data)
  ice_max_timeseries.append(imax)


  icsum = np.nansum(ice_crystal_mmr.data)
  ice_crystal_sum_timeseries.append(icsum)
  icmean = np.nanmean(ice_crystal_mmr.data)
  ice_crystal_mean_timeseries.append(icmean)
  icmin = np.nanmin(ice_crystal_mmr.data)
  ice_crystal_min_timeseries.append(icmin)
  icmax = np.nanmax(ice_crystal_mmr.data)
  ice_crystal_max_timeseries.append(icmax)


  gsum = np.nansum(graupel_mmr.data)
  graupel_sum_timeseries.append(gsum)
  gmean = np.nanmean(graupel_mmr.data)
  graupel_mean_timeseries.append(gmean)
  gmin = np.nanmin(graupel_mmr.data)
  graupel_min_timeseries.append(gmin)
  gmax = np.nanmax(graupel_mmr.data)
  graupel_max_timeseries.append(gmax)


  ssum = np.nansum(snow_mmr.data)
  snow_sum_timeseries.append(ssum)
  smean = np.nanmean(snow_mmr.data)
  snow_mean_timeseries.append(smean)
  smin = np.nanmin(snow_mmr.data)
  snow_min_timeseries.append(smin)
  smax = np.nanmax(snow_mmr.data)
