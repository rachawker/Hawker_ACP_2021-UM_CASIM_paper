...
for t in np.arange(start_time*60,end_time*60-1,dt_output[m]):
  ...
  #dbz_cube=icube[3]
  #print np.amax(dbz_cube.data)

  #print dbz_cube
  dbz1=dbz_cube.collapsed('model_level_number', iris.analysis.SUM)
  dbz=dbz1.data
  print np.amax(dbz)
  ...
  cbar.set_label('Integrated Freezing (cloud droplets/kg/s)',fontsize=15)
...
#  ax.set_extent((-6,-3.2,50,51.2))
  plt.title('Homogeneous 21/08/2015 '+str(time),fontsize=15)

  print 'Save figure'
  fig_name = fig_dir + 'homogeneous_freezing_20150821_'+date+'.png'
...
#  ax.set_extent((-6,-3,49.8,51.2))
#  ax.set_extent((-6,-3.2,50,51.2))
  plt.title('Heterogeneous 21/08/2015 '+str(time),fontsize=15)

  print 'Save figure'
  fig_name = fig_dir + 'heterogeneous_freezing_20150821_'+date+'.png'
  plt.savefig(fig_name)#, format='eps', dpi=300)
  #plt.show()
  plt.close()
...
#  ax.set_extent((-6,-3,49.8,51.2))
#  ax.set_extent((-6,-3.2,50,51.2))
  plt.title('Secondary 21/08/2015 '+str(time),fontsize=15)

  print 'Save figure'
  fig_name = fig_dir + 'secondary_freezing_20150821_'+date+'.png'
  plt.savefig(fig_name)#, format='eps', dpi=300)
  #plt.show()
  plt.close()
...
###Rain
  ...
  cs=ax.pcolormesh(lon,lat,dbz,cmap=cmap,norm=norm,transform=ccrs.PlateCarree())
  cbar=plt.colorbar(cs,orientation='horizontal',extend='both')
  cbar.set_label('Integrated Freezing (raindrops/kg/s)',fontsize=15)
  lats1 = [ 8.9, 16.1, 16.1, 8.9 ]
  lons1 = [ -24.55, -24.55, -16.45, -16.45 ]
  #draw_screen_poly( lats1, lons1)
  level = np.arange(1,800,200)*10**-3
...
#  ax.set_extent((-6,-3,49.8,51.2))
#  ax.set_extent((-6,-3.2,50,51.2))
  plt.title('Raindrop freezing 21/08/2015 '+str(time),fontsize=15)

  print 'Save figure'
  fig_name = fig_dir + 'raindrop_freezing_20150821_'+date+'.png'
  plt.savefig(fig_name)#, format='eps', dpi=300)
  #plt.show()
  plt.close()
