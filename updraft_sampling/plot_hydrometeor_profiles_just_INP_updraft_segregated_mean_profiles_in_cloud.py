...
for r in range(0, len(hydrometeor_number)):
  ...
  for f in range(0,len(list_of_dir)):
    ...
    fr1 = nc1.variables[hydrometeor_number[r]]    
    fr1 = np.asarray(fr1)
    fr1 = fr1[40:,:]
    fr1 = fr1.mean(axis=0)
    ...
    fr2 = nc2.variables[hydrometeor_number[r]]
    fr2 = np.asarray(fr2)
    fr2 = fr2[40:,:]
    fr2 = fr2.mean(axis=0)
    ...
    fr3 = nc3.variables[hydrometeor_number[r]]
    fr3 = np.asarray(fr3)
    fr3 = fr3[40:,:]
    fr3 = fr3.mean(axis=0)
  ...
  axhom.set_xlabel(freezing_title[r]+r" ($\mathrm{m{{^-}{^3}}}$)", fontsize=8, labelpad=10)
  axhet.set_xlabel(freezing_title[r]+r" ($\mathrm{m{{^-}{^3}}}$)", fontsize=8, labelpad=10)
  axsecond.set_xlabel(freezing_title[r]+r" ($\mathrm{m{{^-}{^3}}}$)", fontsize=8, labelpad=10)
  ...
  fig.tight_layout()
  #plt.xscale('log')
  fig_name = fig_dir + freezing_name[r] + '_just_INP_updraft_separated_in_cloud_mean_profiles.png'
...
for r in range(2, 3):
  ...
  axsecond.set_ylim(0,16000)
  if r == 2:
    axhom.set_xlim(1e-2,1e8)
    axhet.set_xlim(1e-2,1e8)
    axsecond.set_xlim(1e-2,1e8)
   # level = np.logspace(-3,8,5)
 # if r == 3:
  ...
  #axhom.set_xticklabels(level)
 # axhet.set_xticklabels(level)
  #axsecond.set_xticklabels(level)
  axhom.ticklabel_format(format='%.0e',axis='x')
  axhet.ticklabel_format(format='%.0e',axis='x')
  axsecond.ticklabel_format(format='%.0e',axis='x')   
  axhom.set_title('updraft < 1m/s', fontsize=8)
  ...
  fig.tight_layout()
  #plt.xscale('log')
  fig_name = fig_dir + freezing_name[r] + '_LOG_SCALE_just_INP_updraft_separated_in_cloud_mean_profiles.png'
  #fig_name = fig_dir + 'hydrometeor_number_timeseries_plot.png'
  plt.savefig(fig_name, format='png', dpi=400)
  plt.show()
  plt.close()
