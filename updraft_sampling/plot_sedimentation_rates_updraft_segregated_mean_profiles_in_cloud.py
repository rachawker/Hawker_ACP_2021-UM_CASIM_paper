...
sedimentation_rates = ['ice_sedimentation_by_z_mean', 'snow_sedimentation_by_z_mean', 'graupel_sedimentation_by_z_mean']
sedimentation_title =['Ice sedimentation', 'Snow sedimentation', 'Graupel sedimentation']
sedimentation_name =['Ice_sedimentation', 'Snow_sedimentation', 'Graupel_sedimentation']
...
for r in range(0, len(sedimentation_rates)):
  ...
  for f in range(0,len(list_of_dir)):
   # if f==0:
    ...
    print data_path
    file1=data_path+'/um/netcdf_summary_files/updraft_sampling/updraft_below_10eminus1_sedimentation_rates_by_max_updraft_timeseries_in_cloud_only.nc'
    nc1 = netCDF4.Dataset(file1)
    fr1 = nc1.variables[sedimentation_rates[r]]    
    fr1 = np.asarray(fr1)
    fr1 = fr1[40:,:]
    frf1 = (fr1.mean(axis=0))/(120*5)

    file2=data_path+'/um/netcdf_summary_files/updraft_sampling/updraft_between_10eminus1_and_10_sedimentation_rates_by_max_updraft_timeseries_in_cloud_only.nc'
    nc2 = netCDF4.Dataset(file2)
    fr2 = nc2.variables[sedimentation_rates[r]]
    fr2 = np.asarray(fr2)
    fr2 = fr2[40:,:]
    frf2 = (fr2.mean(axis=0))/(120*5)

    file3=data_path+'/um/netcdf_summary_files/updraft_sampling/updraft_over_10_sedimentation_rates_by_max_updraft_timeseries_in_cloud_only.nc'
    nc3 = netCDF4.Dataset(file3)
    fr3 = nc3.variables[sedimentation_rates[r]]
    fr3 = np.asarray(fr3)
    fr3 = fr3[40:,:]
  ...
  axsecond.legend([handle for i,handle in enumerate(handles) if i in display]+[simArtist,anyArtist],
          [label for i,label in enumerate(labels) if i in display]+['Hallett Mossop active', 'Hallett Mossop inactive'],bbox_to_anchor=(2.25, 0.35), fontsize=7.5)
  #axsecond.legend(bbox_to_anchor=(1, 0.95), fontsize=8)
  axhom.set_ylabel('Height (m)', fontsize=8)
  axhom.set_xlabel(sedimentation_title[r]+r" ($\mathrm{m{{^-}{^3}} s{{^-}{^1}}}$)", fontsize=8)
  axhet.set_xlabel(sedimentation_title[r]+r" ($\mathrm{m{{^-}{^3}} s{{^-}{^1}}}$)", fontsize=8)
  axsecond.set_xlabel(sedimentation_title[r]+r" ($\mathrm{m{{^-}{^3}} s{{^-}{^1}}}$)", fontsize=8)
  axhom.set_ylim(0,16000)
  axhet.set_ylim(0,16000)
  axsecond.set_ylim(0,16000)

  axhom.ticklabel_format(style='sci',axis='x')
  axhet.ticklabel_format(style='sci',axis='x')
  axsecond.ticklabel_format(style='sci',axis='x')

  axhom.locator_params(nbins=4, axis='x')
  axhet.locator_params(nbins=4, axis='x')
  axsecond.locator_params(nbins=4, axis='x')
  ...
  fig.tight_layout()
  #plt.xscale('log')
  fig_name = fig_dir + sedimentation_name[r] + '_updraft_separated_in_cloud_mean_profiles.png'
  #fig_name = fig_dir + 'sedimentation_rates_timeseries_plot.png'
